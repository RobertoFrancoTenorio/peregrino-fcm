...
export class CitaService {
  ...
  obtenNotificaciones(id: string){
    return this.afs.collection('SegMedico/peregrino/citas', ref =>ref
    .where('detDoctor.id', '==', id).where('estado_notificacion', '==', 'por revisar').orderBy('f_cita', 'asc')).snapshotChanges().pipe(
      map(actions => actions.map(a => {
        const data = a.payload.doc.data()
        let evento = {};
        evento = {
